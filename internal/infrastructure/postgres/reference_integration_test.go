...
func TestReferenceMigrationRoundTrip(t *testing.T) {
	...
	must(t, err)
	down7, err := os.ReadFile("../../../migrations/000007_opening.down.sql")
	must(t, err)
	up7, err := os.ReadFile("../../../migrations/000007_opening.up.sql")
	must(t, err)
	_, err = pool.Exec(ctx, string(down7))
	must(t, err)
	_, err = pool.Exec(ctx, string(down))
	must(t, err)
	_, err = pool.Exec(ctx, string(up))
	must(t, err)
	_, err = pool.Exec(ctx, string(up7))
	must(t, err)
