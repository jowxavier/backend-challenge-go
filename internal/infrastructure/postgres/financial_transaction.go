...
func (r *Runner) WithinFinancialTransaction(ctx context.Context, work func(financial.Repositories) error) error {
	return r.WithinTransaction(ctx, func(repos *Repositories) error {
		return work(financial.Repositories{Outbox: &OutboxWriter{db: repos.WagerTransactions.db}, Wallets: repos.Wallets, Transactions: repos.WagerTransactions, Keys: &IdempotencyRepository{db: repos.WagerTransactions.db}, Ledger: &LedgerRepository{db: repos.WagerTransactions.db}})
	})
}
